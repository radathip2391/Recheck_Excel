...
def process_excel_data(uploaded_file):
            ...
            if MASTER_DB is not None:
                for ad in addr_sets:
                    if c_idx in [ad['p'], ad['d'], ad['s'], ad['z']]:
                        ...
                        d_v = str(df_emp.iloc[r_idx, ad['p']]).strip() # หมายเหตุ: ในโค้ดเดิมท่านเขียน ad['p'] ซ้ำ ผมคงไว้ตามเดิมตามที่ท่านสั่งว่าห้ามแก้ส่วนอื่น
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        # --- จุดแก้ไข: จัดการฟอร์แมตวันที่ก่อนเขียนไฟล์ ---
        for c_idx in DATE_COLS_IDX:
            col_name = df_emp.columns[c_idx]
            df_emp[col_name] = df_emp[col_name].apply(lambda x: smart_date_parser(x)[0].strftime('%d/%m/%Y') if smart_date_parser(x)[1] else x)
