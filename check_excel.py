...
import io
import re
import gc # เพิ่มการจัดการ Garbage Collector
...
if uploaded_file:
    try:
        # ใช้ BytesIO แบบจัดการ memory
        file_bytes = uploaded_file.getvalue()
        
        # อ่านข้อมูลทั้ง 2 ชีท
        df_emp = pd.read_excel(io.BytesIO(file_bytes), sheet_name="พนักงาน")
        df_ref = pd.read_excel(io.BytesIO(file_bytes), sheet_name="รายละเอียด (ห้ามแก้ไข)")
        
        # เตรียมฐานข้อมูลอ้างอิง
        ref_data = {}
        for col in MAP_COLS:
            if col in df_ref.columns:
                # ใช้ set แทน list เพื่อความเร็วในการตรวจสอบ (Search $O(1)$ แทน $O(n)$)
        ...
        if error_details:
            ...
            st.download_button(
                ...
                file_name=f"Check_Result_{datetime.now().strftime('%H%M%S')}.xlsx",
        ...
        # --- ส่วนสำคัญในการจัดการ Memory Leak ---
        # 1. ปิดตัวแปรขนาดใหญ่
        del df_emp
        del df_ref
        del file_bytes
        output.close()
        # 2. บังคับเรียก Garbage Collector เพื่อคืน RAM ให้ระบบ
        gc.collect()
