...
def load_master_db_from_csv():
    ...
    for enc in ['utf-8-sig', 'tis-620']:
        try:
            ...
            # โครงสร้างไฟล์: 0=ไปรษณีย์, 3=ตำบล, 4=อำเภอ, 5=จังหวัด
...
def process_excel_data(uploaded_file):
    ...
    for r_idx in range(len(df_emp)):
        for c_idx in range(len(df_emp.columns)):
            ...
            # 3.3 ระบบตรวจสอบที่อยู่แบบแยกเช็ค (เพื่อให้เช็คครบทุกช่อง)
            if MASTER_DB is not None:
                for ad in addr_sets:
                    # ถ้าคอลัมน์ปัจจุบันอยู่ในเซ็ตที่อยู่นี้
                    if c_idx in [ad['p'], ad['d'], ad['s'], ad['z']]:
                        ...
                        # ตรวจอำเภอ (ต้องสัมพันธ์กับจังหวัด)
                        ...
                        # ตรวจตำบล (ต้องสัมพันธ์กับจังหวัดและอำเภอ)
                        ...
                        # ตรวจรหัสไปรษณีย์ (ต้องสัมพันธ์กับทั้งเซ็ต)
                        if c_idx == ad['z'] and z_v != "" and s_v != "" and d_v != "" and p_v != "":
                            match_z = MASTER_DB[(MASTER_DB['province'] == p_v) & 
                                              (MASTER_DB['district'] == d_v) & 
                                              (MASTER_DB['subdistrict'] == s_v) & 
                                              (MASTER_DB['zipcode'] == z_v)]
                            if match_z.empty:
    ...
    # --- 4. สร้างไฟล์ Output (ปรับปรุงให้รวมทุกชีท) ---
    output = io.BytesIO()
    
    # อ่านชีททั้งหมดจากไฟล์เดิมเตรียมไว้
    try:
        uploaded_file.seek(0)
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        # เขียนชีท 'พนักงาน' ที่ถูกแก้แล้วลงไปก่อน
        df_emp.replace(["nan", "NaN"], "", inplace=True)
        df_emp.to_excel(writer, index=False, sheet_name='พนักงาน')
        
        # วนลูปก๊อปปี้ชีทอื่นๆ ทั้งหมด (รวมถึงชีทรายละเอียด) กลับคืนไป
...
if uploaded_file:
    try:
        ...
        if errs:
            ...
            st.download_button("📥 ดาวน์โหลดไฟล์แก้ไข", data=final_data, file_name=f"Checked_Data.xlsx", use_container_width=True)
